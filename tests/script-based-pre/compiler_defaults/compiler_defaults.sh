...
# Locate kani-compiler and the kani sysroot from the dev install. `kani` is on
# PATH (the test harness puts the install's bin/ there); KANI_HOME is its
# install root — `cargo kani` passes `--sysroot ${KANI_HOME}` (LibConfig::new()).
command -v kani > /dev/null || { echo "ERROR: kani not on PATH (run via the kani test harness or 'cargo build-dev' first)"; exit 1; }
KANI_HOME=$(dirname "$(dirname "$(command -v kani)")")
